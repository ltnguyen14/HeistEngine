...
namespace Heist {
  ...
  bool GUIManager::ButtonP(vec3 rect, vec4 color, const std::string& text, vec4 textColor, real32 scale, real32 padding, bool breakLines) {
    real32 textHeight = 0.0f;
    if (breakLines) {
      textHeight = Text(text, {rect.x + padding, rect.y + padding, rect.z - 2 * padding, 0.0f}, textColor, scale);
    } else {
      textHeight = Text(text, {rect.x + padding, rect.y + padding, rect.z - 2 * padding}, textColor, scale);
    }

    return ButtonP({rect.x, rect.y, rect.z, 2 * padding + textHeight}, color);
  }

  bool GUIManager::ButtonP(vec4 rect, vec4 color, const std::string& text, vec4 textColor, real32 scale, real32 padding, bool breakLines) {
    real32 textHeight = 0.0f;
    if (breakLines) {
      textHeight = Text(text, {rect.x + padding, rect.y + padding, rect.z - 2 * padding, 0.0f}, textColor, scale);
    } else {
  ...
  bool GUIManager::Button(vec2 size, vec4 color, const std::string& text, vec4 textColor, real32 scale, real32 padding, bool breakLines) {
    if (autoLayouts.size() >= 1) {
      GUIAutoLayout& layout = autoLayouts.back();

      bool triggered = ButtonP({layout.xOffset + layout.rect.x, layout.yOffset + layout.rect.y, size.x, size.y}, color, text, textColor, scale, padding, breakLines);
  ...
  bool GUIManager::Button(real32 width, vec4 color, const std::string& text, vec4 textColor, real32 scale, real32 padding, bool breakLines) {
    if (autoLayouts.size() >= 1) {
      GUIAutoLayout& layout = autoLayouts.back();

      bool triggered = ButtonP({layout.xOffset + layout.rect.x, layout.yOffset + layout.rect.y, width}, color, text, textColor, scale, padding, breakLines);
  ...
  real32 GUIManager::Text(const std::string& text, const vec4& rect, const vec4& color, real32 scale) {
    ...
    for (auto character : text) {
      auto charData = fontData->data[character];
	  lineHeight = std::max(lineHeight, (charData.height + charData.yOffset) * scale);
    }
    ...
    for (int32 index = 0; index < text.size(); index++) {
      ...
      auto charData = fontData->data[character];
      Renderer2D::DrawText({cursorPos.x + charData.xOffset * scale, 
							cursorPos.y + charData.yOffset * scale, 
      ...
      cursorPos.x += charData.xAdvance * scale;
  ...
  real32 GUIManager::Text(const std::string& text, const vec3& rect, const vec4& color, real32 scale) {
	...
	for (int32 i = 0; i < text.size(); i++) {
      auto charData = fontData->data[text[i]];
	  // Add width till last character
	  if (i != text.size() - 1) {
		  totalWidth += charData.xAdvance * scale;
	  }
      lineHeight = std::max(lineHeight, (charData.height + charData.yOffset) * scale);
    }
	auto lastCharacterData = fontData->data[text[text.size() - 1]];
	totalWidth += lastCharacterData.xOffset + lastCharacterData.width;
    ...
    for (int32 index = 0; index < text.size(); index++) {
      ...
      Renderer2D::DrawText(
                           { cursorPos.x + charData.xOffset * scale * fitScale,
                               cursorPos.y + charData.yOffset * scale * fitScale,
      ...
      cursorPos.x += charData.xAdvance * scale * fitScale;

    }
    return lineHeight * fitScale;
  }
